...
function ReclamationsList() {
  ...
  if (Array.isArray(data) && data.length === 0)
    return (
      <p>
        Brak danych <Link to="/addReclamation">Dodaj pierwszą reklamalcję</Link>
      </p>
    );
  if (data.length >= 1)
    return (
        ...
        <table>
          ...
          <tbody>
            {data.map((el) => (
              <tr key={el.id}>
                <td>
                  <button value={el.id} onClick={handleEdit}>
                    Edytuj
                  </button>
                </td>
                <td>
                  {(() => {
                    const diff = Math.round(
                      (Date.parse(el.submissionDate) -
                        Date.parse(el.deadlineDate)) /
                        (1000 * 60 * 60 * 24)
                    );
                    return diff > 0
